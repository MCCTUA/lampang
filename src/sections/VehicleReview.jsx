...
const ROWS = [
  { name: 'รถบรรทุก 6 ล้อ', note: 'ตัวทึบ สูงชัด', cam: ['g', 'ดี'], radar: ['g', 'ดี'], lidar: ['g', 'ดีมาก'] },
  { name: 'รถกระบะตู้ทึบ / หลังคาสูง', note: 'รูปทรงชัด', cam: ['g', 'ดี'], radar: ['g', 'ดี'], lidar: ['g', 'ดีมาก'] },
  { name: 'รถกระบะคอกเปล่า สีดำ', note: 'ผิวดำ สะท้อนน้อย', cam: ['m', 'ดำ · คืนต้อง IR'], radar: ['g', 'ไม่ขึ้นกับสี'], lidar: ['b', 'สะท้อนน้อย'] },
  { name: 'รถพาดเหล็ก / ไม้ยาว ยื่นสูง', note: 'บาง · ตัวอย่างน้อย', cam: ['m', 'บาง/ต้องข้อมูล'], radar: ['m', 'รูปทรงบาง'], lidar: ['m', 'จับขอบบนได้'] },
  { name: 'รถบรรทุกโปร่ง / ตาข่าย / นั่งร้าน', note: 'ไม่ทึบ', cam: ['m', 'ต้องเก็บข้อมูล'], radar: ['g', 'ตรวจได้'], lidar: ['m', 'โปร่งอาจทะลุ'] },
  { name: 'รถเก๋ง / กระบะเปล่า (ต่ำ 3.3 ม.)', note: 'ไม่เข้าเกณฑ์', cam: ['g', 'ผ่านปกติ'], radar: ['g', 'ผ่านปกติ'], lidar: ['g', 'ผ่านปกติ'] },
];
  ...
  return (
    <Section id="veh" tone="cream">
      ...
      <div style={{ marginTop: 12 }}>
        {/* header */}
        <div style={{ display: 'grid', gridTemplateColumns: '1.9fr 1fr 1fr 1fr', gap: 9, marginBottom: 7 }}>
          <div />
        ...
        {ROWS.map((r) => (
          <div key={r.name} style={{ display: 'grid', gridTemplateColumns: '1.9fr 1fr 1fr 1fr', gap: 9, alignItems: 'center', marginBottom: 8 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--h-ink)', lineHeight: 1.25 }}>{r.name}</div>
              <div style={{ fontSize: 11.5, color: 'var(--h-muted)' }}>{r.note}</div>
            </div>
      ...
      <p style={{ fontSize: 11.5, color: 'var(--h-muted)', marginTop: 7 }}>
        ระดับเชิงคุณภาพ (ประมาณการ) · ยืนยันด้วยการทดสอบหน้างาน
      </p>
    </Section>
  );
}
